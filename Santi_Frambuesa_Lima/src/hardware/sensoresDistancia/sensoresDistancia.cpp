...
paredes digitalizarParedes(sensado sensadoActual, paredes paredActual){

  //HAY QUE REPENSAR MANUALMENTE ESTA FUNCIÓN DE FILTRADO
  static long tiempoUltimaDigitalizacion = 0;
  static uint16_t cLecturasAnteriores[3] = {0,0,0};
  if(sensadoActual.distanciaIzq < UMBRAL_LECTURA){
      cLecturasAnteriores[0] = cLecturasAnteriores[0] + 1;
    } else {
      cLecturasAnteriores[0] = 0; // ¡Crucial para que la pared desaparezca!
    }
    if(sensadoActual.distanciaCent < UMBRAL_LECTURA){
      cLecturasAnteriores[1] = cLecturasAnteriores[1] + 1;
    } else {
      cLecturasAnteriores[1] = 0; // ¡Crucial para que la pared desaparezca!
    }
    if(sensadoActual.distanciaDer < UMBRAL_LECTURA){
      cLecturasAnteriores[2] = cLecturasAnteriores[2] + 1;
   } else {
      cLecturasAnteriores[2] = 0; // ¡Crucial para que la pared desaparezca!
    }

   if(cLecturasAnteriores[0] > CANT_LECTURAS_IGNORADAS){
      paredActual.paredIzq = 1;
    }
    else{
      paredActual.paredIzq = 0;
    }
    if(cLecturasAnteriores[1] > CANT_LECTURAS_IGNORADAS){
      paredActual.paredCent = 1;
    }
    else{
      paredActual.paredCent = 0;
    }
    if(cLecturasAnteriores[2] > CANT_LECTURAS_IGNORADAS){
      paredActual.paredDer = 1;
    }
    else{
      paredActual.paredDer = 0;
    }
  return paredActual;
}